...
const main = async () => {
    //await newUserQuery();
    let Difference_in_Days_string = await countDownUtil(); //gets the countdown value
    var id = 1;
    while(true){
        var ID : string = ''+id;
        var RecieverEmail : string | undefined =  await QueryForEmailList( "email", ID); //postgresql query function pulls values from the db dynamically
